...
-- SELECT id,shopping_date,total FROM orders
...
-- SELECT DISTINCT o.id,o.shopping_date,o.total FROM orders o JOIN order_detail od
-- ON od.order_id = o.id
-- WHERE od.shipped = FALSE AND o.shopping_date < NOW() - INTERVAL 10 DAY;



-- Display list of shoppers which have not ordered anything since last month.

-- SELECT shopper_id,name,email FROM shopper
...
-- SELECT s.shopper_id,s.name,s.email,o.id,o.shopping_date,o.total FROM shopper s JOIN orders o
...
-- SELECT o.order_id,o.price,o.quantity,p.title,p.description FROM order_detail o JOIN product p
...
-- SELECT od.id,o.id AS order_id,p.title,od.quantity,o.shopping_date FROM order_detail od JOIN product p
